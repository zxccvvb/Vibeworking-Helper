...
function activate(context) {
    ...
    const repoProvider = new treeProviders_1.RepoTreeProvider(service);
    const appProvider = new treeProviders_1.AppTreeProvider(service);
    const taskProvider = new treeProviders_1.TaskTreeProvider(service);
    const docProvider = new treeProviders_1.DocScopeTreeProvider(service);
    const dashboardProvider = new dashboardViewProvider_1.DashboardViewProvider(context, service);
    context.subscriptions.push(vscode.window.registerTreeDataProvider("localLife.repositories", repoProvider), vscode.window.registerTreeDataProvider("localLife.apps", appProvider), vscode.window.registerTreeDataProvider("localLife.tasks", taskProvider), vscode.window.registerTreeDataProvider("localLife.documents", docProvider), vscode.window.registerWebviewViewProvider(dashboardViewProvider_1.DashboardViewProvider.viewType, dashboardProvider), vscode.commands.registerCommand("localLife.refresh", async () => {
        service.refresh();
        await dashboardProvider.render();
    }), vscode.commands.registerCommand("localLife.openPath", async (relativePath) => {
    ...
    }), vscode.commands.registerCommand("localLife.addRepository", async (payload) => {
    ...
    }), vscode.commands.registerCommand("localLife.removeRepository", async (node) => {
        const repo = extractRepoEntry(node);
        if (!repo) {
            vscode.window.showWarningMessage("请选择要卸载的仓库。");
            return;
        }
        const confirmed = await vscode.window.showWarningMessage(`将卸载仓库“${repo.name}”，并移除 submodule 路径 ${repo.path}。`, { modal: true }, "确认卸载");
        if (confirmed !== "确认卸载") {
            return;
        }
        await vscode.window.withProgress({
    ...
    }), vscode.commands.registerCommand("localLife.removeApplication", async (node) => {
        ...
        const warning = app.kind === "repository"
    ...
    }), vscode.commands.registerCommand("localLife.restoreDocScope", async (node) => {
        const scope = extractDocScopeEntry(node);
        if (!scope) {
            vscode.window.showWarningMessage("请选择要还原的文档目录。");
            return;
        }
        const confirmed = await vscode.window.showWarningMessage(`将把 ${scope.path} 还原到当前 Git 版本，并清理该目录下未跟踪文件。`, { modal: true }, "确认还原");
        if (confirmed !== "确认还原") {
            return;
        }
        await vscode.window.withProgress({
    ...
    }), vscode.commands.registerCommand("localLife.searchGitLabProjects", async (payload) => {
    ...
    }), vscode.commands.registerCommand("localLife.pullGitLabProjects", async (payload) => {
...
function deactivate() { }
function registerWatchers(context, service) {
...
function showGitLabPullSummary(result) {
    const parts = [];
...
function extractRepoEntry(node) {
    if (!node) {
        return undefined;
    }
    if (node instanceof treeProviders_1.RepoNode) {
        return node.repoEntry;
    }
    return "path" in node && "url" in node ? node : undefined;
}
function extractAppEntry(node) {
    if (!node) {
        return undefined;
    }
    if (node instanceof treeProviders_1.AppNode) {
        return node.appEntry;
    }
    return "repoPath" in node && "path" in node ? node : undefined;
}
function extractDocScopeEntry(node) {
    if (!node) {
        return undefined;
    }
    if (node instanceof treeProviders_1.DocScopeNode) {
