...
class DashboardViewProvider {
    context;
    service;
    static viewType = "localLife.dashboard";
    view;
    gitlabState = {
        form: {
            token: "",
        ...
        view.webview.onDidReceiveMessage(async (message) => {
            switch (message.type) {
                ...
                case "add-repo":
                    ...
                    return;
                case "search-gitlab":
                    await this.searchGitLabProjects(message.payload);
                    return;
                case "pull-gitlab":
                    await this.pullGitLabProjects(message.payload);
                    return;
                default:
                    return;
            }
    ...
    async searchGitLabProjects(input) {
        ...
        try {
            const results = (await vscode.commands.executeCommand("localLife.searchGitLabProjects", input));
    ...
    async pullGitLabProjects(input) {
        ...
        try {
            const result = (await vscode.commands.executeCommand("localLife.pullGitLabProjects", input));
...
function getHtml(webview, data, gitlabState) {
    const nonce = getNonce();
    const locationsMarkup = data.locations
        .map((location) => `<option value="${escapeHtml(location.value)}" ${location.value === gitlabState.form.targetBase ? "selected" : ""}>${escapeHtml(location.label)}${location.description ? ` · ${escapeHtml(location.description)}` : ""}</option>`)
        .join("");
